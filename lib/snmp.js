    ...
    function main(loader, res) {
        function SNMP(stage) {
            ...
            this.createPort = function (name, color, width, height, position, titleText, portCount, sw) {
                var graphics = new PIXI.Graphics();
                var graphics = new PIXI.Sprite(res['port2'].texture);
                //graphics.beginFill(color);
                //graphics.drawRect(0, 0, width, height);
                ...
                var title = this.createTitle(titleText, 0xedef4d, sprite.width, sprite.height, position);
                title.visible = false;
	    ...
	    this.isPortTop = function(port, p) {
                position ={};
                if(port.portPosition == "top") {
                    position.y = p.y - port.height;
                } else if (port.portPosition == "bottom"){
                    position.y = p.y + port.height + port.height*20/100;
                }
                position.x = p.x;
                return position;
            ...
            this.updateLinkPorts = function (graphics, from, to, fromPort, toPort, fromSwitch, toSwitch) {
                ...
                let positions = [];
		        //from = this.isPortTop(fromPort, from);
