...
function drawNetworkTopology() {
    ...
    function main(loader, res) {
        function SNMP(stage) {
            ...
            this.createPort = function (name, color, width, height, position, titleText, portCount, sw) {
                var graphics = new PIXI.Graphics();
                graphics.beginFill(color);
                graphics.drawRect(0, 0, width, height);
                ...
                graphics.endFill();
                graphics.setColor = function (color, status, isSend) {
                    graphics.clear();
                    graphics.beginFill(color);
                    graphics.drawRect(0, 0, width, height);
                    if(isSend) {
            ...
            this.createSwitch = function (swName, portCount, text, enablePortsList) {
                ...
                for (var row = 0; row < 3; row++) {
                    for (var i = 0; i < count; i++) {
                        ...
                        if (row == 0) {
                            var port = this.createPort(name, portColor, portSize, portSize, "top", "tmp", portCount, container);
                            port.x = (dis * i)-port.width/2;
                            container.addChild(port);
                            index++;
                        } else if (row == 2) {
                            var port = this.createPort(name, portColor, portSize, portSize, "bottom", "tmp", portCount, container);
            ...
            this.getLinePositions = function (from, to, fromPort, toPort, fromSwitch, toSwitch, positions) {
                if(this.isFromPoint(fromSwitch, fromPort, from)) {
                    let fromT = this.calculatePartLine(from, fromPort, toPort.getLineDist());
                    positions.push({ x: fromT.x, y : fromT.y });
                    this.getLinePositions(fromT, to, fromPort, toPort, fromSwitch, toSwitch, positions);
                    return;
                }
                if(from.x != to.x) {
                    let p = this.getAvailableXPosition(from, to);
                    if(p.x != to.x) {
                        p.x = to.x > p.x ? p.x -= 10 : p.x+= 10;
                    }
                    positions.push({ x: p.x, y : p.y });

                    if(p.x != to.x) {
                        let py = p.switch.y < p.y ? p.y + p.switch.H + 20 : p.y - p.switch.H - 20;
                        p = {x: p.x, y : py};
                        positions.push({ x: p.x, y : p.y });

                    }

                    this.getLinePositions(p, to, fromPort, toPort, fromSwitch, toSwitch, positions);
                    return;
                }
...
/*            this.getLinePositions = function (from, to, fromPort, toPort, fromSwitch, toSwitch, positions) {
                from1 = this.calculatePartLine(from, fromPort, toPort.getLineDist());
            ...
            this.updateLinkPorts = function (graphics, from, to, fromPort, toPort, fromSwitch, toSwitch) {
                ...
                this.getLinePositions(from, to, fromPort, toPort, fromSwitch, toSwitch, positions);
