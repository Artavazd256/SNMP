...
function drawNetworkTopology() {
    ...
    var getChildByName = function (list, name) {
    ...
    var calculatePosition = function (element, position) {
    ...
    function main(loader, res) {
        function SNMP(stage) {
            ...
            this.popupItems = [
                ...
                {action: enableCon, text: 'Disable', src: 'image/disable.png', name: 'disable'},
                {action: cancelCon, text: 'Enable', src: 'image/enable.png', name: 'enable'},
            ];
            ...
            this.showPopupMenu = function () {
                var popupMenu = this.getChildByName("popupMenu");
                var info = stage.getChildByName(this.titleName);
            ...
            this.createPopupMenu = function (color,  position) {
                ...
                calculatePosition(popupMenu, position);
                return popupMenu;
            };
            ...
            this.createBodyOfSwitch = function (name, text, color, width, height, sw) {
                ...
                var box = snmp.createBox1(name, color, content.width, height);
            ...
            this.onDragMove = function() {
                if (this.dragging && this.isPort  == null) {
            ...
            this.createSwitch = function (swName, portCount, text) {
                ...
                for (var row = 0; row < 3; row++) {
                    for (var i = 0; i < count; i++) {
                        ...
                        if (row == 0) {
                            var port = this.createPort(name, 0x00ff00, portSize, portSize, "top", "tmp", portCount);
                            port.x = (dis * i)-port.width/2;
                            container.addChild(port);
                            index++;
                        } else if (row == 2) {
                            var port = this.createPort(name, 0x00ff00, portSize, portSize, "bottom", "tmp", portCount);
...
/*                let sw = drawNetworkTopology.switchListObject;
            ...
            this.createLine = function (from, to, size) {
                ...
                //graphics.beginFill(0x00FF00);
                graphics.lineStyle(size, 0x00ff00, 1);
        ...
        if(drawNetworkTopology.switchList !== undefined && drawNetworkTopology.netList !== undefined) {
            ...
            for(let i = 0; i < switchList.length; i++) {
                ...
                var sw = snmp.createSwitch(name, portCount, description);
                sw.x = sw.width * i;
                sw.y = sw.height * i;
                stage.addChild(sw);
